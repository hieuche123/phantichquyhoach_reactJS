...
const mapScale = 25000 // tỷ lệ của ảnh bản đồ

const newMapScale = (widthImage / (center.lng * Math.cos(center.lat * (Math.PI / 180)))) / 2

// Chuyển đổi kích thước ảnh sang đơn vị thực tế (mét)
const newWidthInMeters = widthImage / (newMapScale * 0.0254) // chuyển đổi pixel sang mét
const newHeightInMeters = heightImage / (newMapScale * 0.0254)

// Tính toán lại `imageBounds`
const newImageBounds = {
  north: center.lat + (newHeightInMeters / 2),
  south: center.lat - (newHeightInMeters / 2),
  east: center.lng + (newWidthInMeters / 2),
  west: center.lng - (newWidthInMeters / 2)
}
